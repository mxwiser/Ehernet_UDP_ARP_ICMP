...
`timescale 1 ps / 1 ps
module flasher_asmi_parallel2_top_0_asmi2_core_csr_controller #(
		...
		if (reset) begin 
			...
			if (write_csr_control) begin
				if (avl_wrdata[0] == 1)
					...
					csr_control_data_reg <= avl_wrdata; // Change made to ensure that SPI signals can be enabled once again after being disabled within the Control register of the IP's CSR
			end
		end
	end
	...
	// send this value to export chip select interface so that the XIP can use
   	assign chip_select 		= csr_control_data_reg[7:4];
...
// output_name 					: flasher_asmi_parallel2_top_0_asmi2_core_csr_controller
